...
directory = 'alpha_test_sod-shock_16-58-02'
...
def plot_variables(var_dict, name, t_vals, var_vals, save=False):
    ...
    for t_val in t_vals:
        for count, (key, value) in enumerate(var_dict.items()):
            ...
            # print(var)
            if t == t_val:
                var_lab = str(round(float(var), 5))
                plt.plot(value, label=var_lab, lw=2, color=color_assign[var])
        plt.grid()
        t_val= str(round(float(t_val), 5))
