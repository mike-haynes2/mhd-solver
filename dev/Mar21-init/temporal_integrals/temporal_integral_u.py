...
def Temporal_Integral_U(w_n, rho, B, p): # time is given by variable n
    """
    ...
    for j in range(1, len(w_n)-1):
        ...
        ### f_j ###
        # calculate deltas
        f_j = config.f_NS_1D(w_n[j], rho, B, p)
        f_jm1 = config.f_NS_1D(w_n[j-1], rho, B, p)
        f_jp1 = config.f_NS_1D(w_n[j+1], rho, B, p)
        ...
        if j == len(w_n)-2: # account for problematic j + 2 index
            ...
            f_jp1 = config.f_NS_1D(w_n[j+1], rho, B, p)
            f_j = config.f_NS_1D(w_n[j], rho, B, p)
            f_jp2 = config.f_NS_1D(w_n[j+2], rho, B, p)
        ...
        I_j = lam * (config.f_NS_1D(w_n[j] - lam/2 * f_prime_j, rho, B, p) + config.f_NS_1D(w_n[j+1] - lam/2 * f_prime_jp1, rho, B, p))
        w_np1[j] = I_j
