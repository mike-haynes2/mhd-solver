...
def Temporal_Integral_Energy(w_n, rho, u, B, p): # time is given by variable n
    """
    ...
    for j in range(1, len(w_n)-1):
        ...
        if j == len(w_n)-2: # account for problematic j + 2 index
            ...
            f_jp1 = config.f_energy_1D(w_n[j+1], rho, u, B, p)
            f_j = config.f_energy_1D(w_n[j], rho, u, B, p)
            f_jp2 = config.f_energy_1D(w_n[j+2], rho, u, B, p)
        ...
        I_j = lam * (config.f_energy_1D(w_n[j] - lam/2 * f_prime_j, rho, u, B, p) + config.f_energy_1D(w_n[j+1] - lam/2 * f_prime_jp1, rho, u, B, p))
        w_np1[j] = I_j
