...
def Temporal_Integral_Rho(w_n, u): # time is given by variable n
    """
    ...
    for j in range(1, len(w_n)-1):
        ...
        ### f_j ###
        # calculate deltas
        f_j = config.f_continuity_1D(w_n[j], u)
        f_jm1 = config.f_continuity_1D(w_n[j-1], u)
        f_jp1 = config.f_continuity_1D(w_n[j+1], u)
        ...
        if j == len(w_n)-2: # account for problematic j + 2 index
            ...
            f_jp1 = config.f_continuity_1D(w_n[j+1], u)
            f_j = config.f_continuity_1D(w_n[j], u)
            f_jp2 = config.f_continuity_1D(w_n[j+2], u)
        ...
        I_j = lam * (config.f_continuity_1D(w_n[j] - lam/2 * f_prime_j, u) + config.f_continuity_1D(w_n[j+1] - lam/2 * f_prime_jp1, u)) 
        w_np1[j] = I_j
